...
select f_name,f_price from fruits where not(f_price >= 2.00 and f_price <= 10.20);
-- 查找所有以’b’字母开头的水果
...
select * from employee e where e.e_salary=(select max(ei.e_salary) from employee ei where e.dept_no=ei.dept_no);
-- 在employee表中，计算每个部门各有多少名员工。
select e_job,count(*) num from employee group by e_job;
-- 在employee表中，计算不同类型职工的总工资数。
select e_job,sum(e_salary) sum from employee group by e_job;
-- 在employee表中，计算不同部门的平均工资。
select e_job,avg(e_salary) avg from employee group by e_job;
...
select f_price,f_name from fruits where (s_id=101 or s_id=102) and f_name='apple';
...
select o_num,group_concat(f_id) from orderitems group by o_num;
-- 在orderitems表中查询30005号订单一共购买的水果总量
select sum(quantity) sum from orderitems where o_num=30005;
-- 在orderitems表中，使用SUM()函数统计不同订单号中订购的水果总量
select o_num,sum(quantity) sum from orderitems group by o_num;
-- 在fruits表中，查询s_id=103的供应商的水果价格的平均值
select avg(f_price) avg from fruits where s_id=103;
...
select max(f_name) max from fruits;
-- 在fruits表中查找市场上价格最低的水果
select f_name from fruits where f_price =(select min(f_price)from fruits);
